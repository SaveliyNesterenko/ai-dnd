...
> Статус: проект работает на package-based backend, SQLite, `/api/v1` и
> React-интерфейсах. Код прототипа исключён из публичного репозитория; для
> переноса существующих кампаний поддерживается отдельный импорт данных.
...
При первом ходе XTTS v2 загрузит модель. Если пакет или голосовой образец
персонажа недоступен, spectator автоматически использует текстовый режим.
...
есть флаги `llm_enabled` и `stt_enabled` (сами ключи наружу не отдаются).
