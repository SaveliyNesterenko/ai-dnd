...
2.  **[JSON PATCH]**
    ...
    *   Ты можешь изменять **только** поля внутри объектов `stats` и `inventory_ops`.
    *   Ключами верхнего уровня в JSON-объекте должны быть ID персонажей (например, `kael_roan`), которые были затронуты событием.
    *   Для каждого `char_id` в значении должен быть объект, содержащий **только те поля, которые изменились** (`stats` или `inventory_ops`).
    *   Для `stats`: можно изменять `hp`, `mp`, `attributes`, `status_effects`. При изменении `hp` или `mp`, передавай весь объект `{"current": ..., "max": ...}`.
    *   Для `inventory`: передавай **только изменения** через `inventory_ops` (список операций). **Не передавай весь массив**.
        Доступные операции:
        - `add`: `{ "op": "add", "item": { "name": "...", "quantity": 1, "description": "..." } }`
        - `update`: `{ "op": "update", "name": "...", "fields": { "quantity": 2, "description": "..." } }`
        - `remove`: `{ "op": "remove", "name": "..." }`
        - `adjust`: `{ "op": "adjust", "name": "...", "quantity_delta": -1 }`
    ...
    **[ПРИМЕР]**
    ...
    [JSON PATCH]
    {
        "silia": {
            ...
            "inventory_ops": [
                { "op": "remove", "name": "Рунический ключ" },
                { "op": "adjust", "name": "Аптечка-регенератор", "quantity_delta": -1 }
            ]
        },
        "kael_roan": {
            "inventory_ops": [
                { "op": "add", "item": { "name": "Рунический ключ", "quantity": 1, ... } }
            ]
        }
    }
    **[/ПРИМЕР]**"""
