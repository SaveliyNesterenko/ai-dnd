...
`finalizing` и запускает durable job Архивариуса. Результат LLM остаётся черновиком и
не изменяет память. GM может отредактировать его, повторить генерацию или ввести итог
вручную.
